...
# SELECT DATA.YYYYMM,(((0.008887 * SUM(DAYSINMONTH*ALLGASMILES/EPATMPG)) * 117538000/COUNT(DISTINCT DATA.HOUSEID)) - (SUM((CAST(electricity.Value AS float)/CAST(mkwh.Value AS float)) * (DAYSINMONTH * EMILES/(EPATMPG * 0.090634441))) + 0.008887 * SUM(DAYSINMONTH*GMILES/EPATMPG)) * 117538000/COUNT(DISTINCT DATA.HOUSEID)) AS DELTACO2EMISSIONS
# FROM(
# 	SELECT daytrip.TDAYDATE AS YYYYMM, daytrip.HOUSEID AS HOUSEID, daytrip.VEHID AS VEHID,SUM(daytrip.TRPMILES) AS ALLGASMILES, EPATMPG,
