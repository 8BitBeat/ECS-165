...
with open('/home/cjnitta/ecs165a/EIA_CO2_Electricity_2015.csv', 'r') as csvfile:
	cur.execute("CREATE TABLE electricity_misc(MSN varchar(20), Column_Order varchar(5), Description varchar(100), Unit varchar(50), CONSTRAINT PK_electricity_misc PRIMARY KEY (MSN));")
	cur.execute("CREATE TABLE electricity( MSN varchar(20), YYYYMM int, Value varchar(20), CONSTRAINT PK_electricity PRIMARY KEY (MSN, YYYYMM), CONSTRAINT FK_electricity_em FOREIGN KEY (MSN) REFERENCES electricity_misc (MSN));")    
	...
	for row in reader:
		...
		if(row[0] != prev_msn):
			prev_msn = row[0]
			misc_query = "INSERT INTO electricity_misc ( MSN, Column_Order, Description, Unit ) VALUES ( \'"+ row[0] +"', " + row[3] + ", '" + row[4] + "', '" + row[5]  +"' ); "
			try:
...
with open('/home/cjnitta/ecs165a/EIA_CO2_Transportation_2015.csv', 'r') as csvfile:
	cur.execute("CREATE TABLE transportation_misc(MSN varchar(20), Column_Order varchar(5), Description varchar(100), Unit varchar(50), CONSTRAINT PK_transportation_misc PRIMARY KEY (MSN));")
	cur.execute("CREATE TABLE transportation( MSN varchar(20), YYYYMM int, Value varchar(20), CONSTRAINT PK_transporation PRIMARY KEY (MSN, YYYYMM), CONSTRAINT FK_transportation_tm FOREIGN KEY (MSN) REFERENCES transportation_misc(MSN));")    
	...
	for row in reader:
		...
		if(row[0] != prev_msn):
			prev_msn = row[0]
			misc_query = "INSERT INTO transportation_misc ( MSN, Column_Order, Description, Unit ) VALUES ( \'" + row[0] + "', " + row[3] + ", '" + row[4] + "', '" + row[5]  +"' ); "
			try:
				cur.execute(misc_query)
			except:
				print "fail 2"
...
with open('/home/cjnitta/ecs165a/EIA_MkWh_2015.csv', 'r') as csvfile:
	cur.execute("CREATE TABLE mkwh_misc(MSN varchar(20), Column_Order varchar(5), Description varchar(100), Unit varchar(50), CONSTRAINT PK_mkwh_misc PRIMARY KEY (MSN));")
	cur.execute("CREATE TABLE mkwh( MSN varchar(20), YYYYMM int, Value varchar(20), CONSTRAINT PK_mkwh PRIMARY KEY (MSN, YYYYMM), CONSTRAINT FK_mm FOREIGN KEY (MSN) REFERENCES mkwh_misc (MSN));")    
	...
	for row in reader:
		...
		if(row[0] != prev_msn):
			prev_msn = row[0]
			misc_query = "INSERT INTO mkwh_misc ( MSN, Column_Order, Description, Unit ) VALUES ( \'"+ row[0] +"', " + row[3] + ", '" + row[4] + "', '" + row[5]  +"' ); "
			try:
				cur.execute(misc_query)
			except:
				print "fail 3"
...
with open('/home/cjnitta/ecs165a/VEHV2PUB.CSV', 'r') as csvfile:
	...
	print "vehicle"
	try:
		cur.execute("CREATE TABLE Vehicle(HOUSEID int, WTHHFIN varchar(20), VEHID int, DRVRCNT int, HHFAMINC int, HHSIZE int, HHVEHCNT int, NUMADLT int, FLAG100 int, CDIVMSAR int, CENSUS_D int, CENSUS_R int, HHSTATE varchar(2), HHSTFIPS int, HYBRID int, MAKECODE varchar(5), MODLCODE varchar(5), MSACAT int, MSASIZE int, OD_READ int, RAIL int, TRAVDAY int, URBAN int, URBANSIZE int, URBRUR int, VEHCOMM int, VEHOWNMO int, VEHYEAR int, WHOMAIN int, WRKCOUNT int, TDAYDATE int, VEHAGE int, PERSONID int, HH_HISP int, HH_RACE int, HOMEOWN int, HOMETYPE int, LIF_CYC int, ANNMILES float, HBHUR varchar(2), HTRESDN int, HTHTNRNT int, HTPPOPDN int, HTEEMPDN int, HBRESDN int, HBHTNRNT int, HBPPOPDN int, BEST_FLG int, BESTMILE float, BEST_EDT int, BEST_OUT int, FUELTYPE int, GSYRGAL int, GSCOST varchar(20), GSTOTCST int, EPATMPG float, EPATMPGF float, EIADMPG float, VEHTYPE int, HH_CBSA varchar(5), HHC_MSA varchar(5), CONSTRAINT PK_vehicle PRIMARY KEY (HOUSEID, VEHID),CONSTRAINT FK_vehicle_household FOREIGN KEY (HOUSEID) REFERENCES household(HOUSEID));")
	except:
		print "failure dude"
